...
//두번째 썸네일에 마우스올리면 큰 이미지를 두번째 썸네일 이미지로 변경하기
...
bookMark.addEventListener('click',()=>{
    ...
    if(loginStatus == 'false'){//위 변수 이용한 조건 분기
        ...
        location.href = './wish.html'//로그아웃 시 login.html 이동
    }
})
...
priceinfoBtn.addEventListener('click',()=>{
    //infoBoolean의 상태를 클릭할때마다 값(infoBoolean의) 반전
...
// 5. 주문 목록 초기 숨기기
// 6. 사이즈select 초기 비활성화(색상 선택 시 활성화) size_select
...
// * 생성한 태그가 li면 ul or ol 의 마지막 자식 위치로 삽입
...
// 9. 주문목록 + 클릭 시 재고수량까지 주문수량+주문금액 표시
// 필요목록 : +버튼, 재고수량(productOptDB[0].stock), 주문수량, 주문금액(orderPrice,order_price), 증가 숫자 데이터
