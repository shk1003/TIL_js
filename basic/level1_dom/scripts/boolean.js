...
console.log(!"A")
let d; //없는 값
result = !d; //nudefined상태에서 반전
console.log(result) //결과는 참
let e = [] //빈배열이라 참
result = !e; //반전해서 거짓
e += ['사과'] 
...
if(x !== true){ console.log('y는 참이다2'); }
if(x && y){ console.log('x, y는 참이다3'); }
...
ageBtn.addEventListener('click',()=>{
    console.log(ageInput.value)
    //ageInput 변수의 value 값이 숫자 10 이하면(조건문)
    //ageResult 변수에 '탑승불가' 문자데이터 출력(조건문 참일때)
    //if(조건식){조건식 결과가 참일때 실행결과}
    //if(ageInput 변수의 value 값이 숫자 10 이하){'탑승불가' 문자데이터}
...
//로그인 조건문
//아이디(미입력) 비밀번호(입력) 로그인 클릭 시 - 아이디 입력경고 메세지 출력
//아이디(입력) 비밀번호(미입력) 로그인 클릭 시 - 비번 입력경고 메세지 출력
...
loginBtn.addEventListener('click',()=>{
    ...
    }else if(userId.value == userDB[0].id && userPw.value == userDB[0].pw){//입력한 id가 관리자의 저장된 id 동일 시 index 이동
