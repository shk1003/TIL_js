...
console.log('----------------------')
for(let i=0; i<3; i++){ console.log(`JS ${i+1}일차`)}//이거 사용
console.log('----------------------')
...
/* let cssQ = prompt('ox퀴즈 : display none은 요소를 화면에서 보이지 않게 하지만, 공간은 차지한다.')
while(cssQ != 'x'){
    cssQ = prompt('ox퀴즈 : display none은 요소를 화면에서 보이지 않게 하지만, 공간은 차지한다.')
}
alert('정답입니다') */ 
...
for(let i=2; i<10; i++){//1단 for 시작
    ...
    }//2단 for 종류(2단 for가 모두 반복 후 1단 다시 시작)
